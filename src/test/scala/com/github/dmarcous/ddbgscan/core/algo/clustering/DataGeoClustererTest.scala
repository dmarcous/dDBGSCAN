...
class DataGeoClustererTest extends FlatSpec{
  ...
  val ungroupedPartitionedData = partitionedData.flatMapGroups{case(key, vals) => (vals.map(instance => (key,instance)))}.collect()
  ...
  "clusterLocalGeoData" should " geo cluster given instances with regard to types (core, border, noise) " in
  {
    ...
    //    (1521455263322734592,ClusteringInstance(1,0,false,4,(34.777112,32.0718015),[8.0,9.0])) //C2 - Address - Haimvelish1-7
    //    (1521455263322734592,ClusteringInstance(1,0,false,4,(34.777547,32.072729),[11.0,2.0]))//C2 - Address - BenTsiyon22-28
    //    (1521455263322734592,ClusteringInstance(1,0,false,4,(34.777558,32.072565),[3.0,4.0]))//C2 - Address - Warburg9-3
    ...
    val clusteredData = DataGeoClusterer.clusterLocalGeoData(
      VOODO_CELL_KEY,
      voodooCellData.map(_._2).toIterator,
      params)
